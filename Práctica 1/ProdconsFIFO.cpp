...
//g++ -std=c++11 -pthread -I. -o ProdconsLIFO ProdconsLIFO.cpp Semaphore.cpp
...
int p_libre=0;   ///LIFO/FIFO
int p_ocupada=0; //FIFO
...
void funcionHebraConsumidora(){
  for(int i=0; i<num_items; i++){
    ...
    int b;
    b=buffer[p_ocupada];
    p_ocupada+=1; //Posición ya cargada, incremento p_ocupada
    if(p_ocupada>=10){
      p_ocupada=p_ocupada%10;
    }
