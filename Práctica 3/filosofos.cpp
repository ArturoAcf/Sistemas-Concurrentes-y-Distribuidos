...
void funcion_filosofos( int id )
{
  ...
  while ( true )
  {
    if(id==0){ // Filósofo que obtendrá los tenedores en orden inverso
      cout <<"Filósofo " <<id <<" solicita el ten. der." <<id_ten_der <<endl;
      MPI_Ssend(&pedir, 1, MPI_INT, id_ten_der, 0, MPI_COMM_WORLD);

      cout <<"Filósofo " <<id << " solicita ten. izq." <<id_ten_izq <<endl;
      MPI_Ssend(&pedir, 1, MPI_INT, id_ten_izq, 0, MPI_COMM_WORLD);
    }else{ // Resto de filósofos
      cout <<"Filósofo " <<id << " solicita ten. izq." <<id_ten_izq <<endl;
      MPI_Ssend(&pedir, 1, MPI_INT, id_ten_izq, 0, MPI_COMM_WORLD);
...
/*
Filósofo 0 solicita el ten. der.9
